...
for id_num in range(0, 5001):
    ...
    # API 1: querySearch
    query_url = f"{base_url}?fname=sentence&id={id_num}&lang=zh_tw"
    try:
        ...
        for translation_entry in data1["Record"]:
            text = translation_entry.get("translation")
            if text:
                output.append({
                    "count": f"{count:04}",
                    "id": sign_id,
...
# 儲存為 JSON 檔案
with open("tsl_signs_sen.json", "w", encoding="utf-8") as f:
